...
$(function(){
	MainPartMap();
	setIntervalAllData(3000);
})

function setIntervalAllData(millisec){
	setInterval(MainDataUpdate, millisec);
	setInterval(MainOtherDataUpdate, millisec);
...
function MainDataUpdate(){
	var sysIn = RoundNum($('#sysInNum').val());
	var visit = RoundNum($('#VisitNum').val());
	$('#sysInNum').val(sysIn);
	$('#VisitNum').val(visit);
	displayNum(sysIn, $('#SysIn'), 5, false);
	displayNum(visit, $('#Visit'), 7, true);
}
function MainOtherDataUpdate(){
	$('#otherData .business span').text(parseInt(6+Math.random()*100));
	$('#otherData .internet span').text(parseInt(6+Math.random()*10000));
}
function RoundNum(start){
	start = parseInt(start);
	return parseInt(start+Math.random()*10000);
}
